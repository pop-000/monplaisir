...
if (isset($_REQUEST['dontsave']) && 'Y' == $_REQUEST['dontsave'])
{
	$intLockUserID = 0;
	$strLockTime = '';
...
if ($REQUEST_METHOD == "POST" && $save_order_data == "Y" && empty($dontsave) AND $saleModulePermissions >= "U" AND check_bitrix_sessid())
{
	...
	if ('' == $errorMessage AND $ID > 0)
	{
		...
		if (isset($apply) AND strlen($apply) > 0)
			LocalRedirect("/bitrix/admin/sale_order_new.php?lang=".LANGUAGE_ID."&ID=".$ID."&LID=".CUtil::JSEscape($LID));
	}
	if ('' != $errorMessage)
		$bVarsFromForm = true;
}
...
if (isset($ORDER_AJAX) AND $ORDER_AJAX == "Y" AND check_bitrix_sessid())
{
	...
	*/
	if (isset($userId) AND isset($buyerType) AND (!isset($profileDefault) OR $profileDefault == ""))
	{
		...
		$arViewedResult = fDeleteDoubleProduct($arViewed, $arFilterRecommended, 'N');
	...
	*/
	if (isset($product) && isset($user_id))
	{
		...
		if (!empty($arOrder["BASKET_ITEMS"]))
		{
			...
			if (!empty($arCatalogProduct) && $bUseCatalog)
			{
				...
				while ($arItem = $rsItems->Fetch())
				{
					...
					$arData[$strKey]["BALANCE"] = doubleval($arItem['QUANTITY']);
				}
			}
		}
		...
		$arData[0]["PRICE_WEIGHT_FORMAT"] = roundEx(DoubleVal($arOrder["ORDER_WEIGHT"]/$WEIGHT_KOEF), SALE_WEIGHT_PRECISION)." ".$WEIGHT_UNIT;
		$arData[0]["PRICE_WEIGHT"] = roundEx(DoubleVal($arOrder["ORDER_WEIGHT"]/$WEIGHT_KOEF), SALE_WEIGHT_PRECISION);
		...
		if ($recommendet == "Y")
		{
			$arRecommended = CSaleProduct::GetRecommendetProduct($userId, $LID, $arFilterRecommended);
...
elseif (isset($ID) AND $ID > 0)
{
	...
	$dbBasket = CSaleBasket::GetList(
		array("SET_PARENT_ID" => "DESC", "TYPE" => "DESC", "ID" => "ASC"),
		array("ORDER_ID" => $ID),
		false,
		false,
		array(
	...
	while ($arBasket = $dbBasket->GetNext())
	{
		...
		if ($productProvider = CSaleBasket::GetProductProvider($arBasket))
		{
			...
			if ($storeCount > 0)
			{
				$arProductStore = $productProvider::GetProductStores(array("PRODUCT_ID" => $arBasket["PRODUCT_ID"], "SITE_ID" => $LID));
...
foreach ($arBasketItem as $key => &$arItem)
{
	if (!empty($arProductData[$arItem["PRODUCT_ID"]]))
	{
		...
		{
			$arBasketItem[$key]["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$arItem["PRODUCT_ID"]
					."&type=".$arProductData[$arItem["PRODUCT_ID"]]["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID="
					.$arProductData[$arItem["PRODUCT_ID"]]["IBLOCK_ID"]."&find_section_section=".$arProductData[$arItem["PRODUCT_ID"]]["IBLOCK_SECTION_ID"];
		}
...
?>
	...
	$hidden = (filter_var($priceDeliveryDiff, FILTER_SANITIZE_NUMBER_INT) != "") ? "" : 'style="display:none"';
	?>
...
<tr>
	<td colspan="2" id="ID_BASKET_CONTAINER">
		...
		if(!empty($_REQUEST["productDelay"]) || !empty($_REQUEST["productSub"]) || !empty($_REQUEST["productNA"]))
		{
			...
			if(!empty($_REQUEST["productSub"]))
			{
				...
				while($arItem = $dbItem->Fetch())
					echo "<b>"."<a href=\"/bitrix/admin/iblock_element_edit.php?ID=".$arItem["ID"]."&type=catalog&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arItem["IBLOCK_ID"]."&find_section_section=".$arItem["IBLOCK_SECTION_ID"]."\">".htmlspecialcharsbx($arItem["NAME"])."</a></b> (".GetMessage("NEWO_PRODUCTS_SUB").")<br />";
			}
			if(!empty($_REQUEST["productDelay"]))
			{
				...
				while($arItem = $dbItem->Fetch())
					echo "<b>"."<a href=\"/bitrix/admin/iblock_element_edit.php?ID=".$arItem["ID"]."&type=catalog&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arItem["IBLOCK_ID"]."&find_section_section=".$arItem["IBLOCK_SECTION_ID"]."\">".htmlspecialcharsbx($arItem["NAME"])."</a></b> (".GetMessage("NEWO_PRODUCTS_DELAY").")<br />";
			}
			if(!empty($_REQUEST["productNA"]))
			{
				...
				while($arItem = $dbItem->Fetch())
					echo "<b>"."<a href=\"/bitrix/admin/iblock_element_edit.php?ID=".$arItem["ID"]."&type=catalog&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arItem["IBLOCK_ID"]."&find_section_section=".$arItem["IBLOCK_SECTION_ID"]."\">".htmlspecialcharsbx($arItem["NAME"])."</a></b> (".GetMessage("NEWO_PRODUCTS_NA").")<br />";
			}
			echo EndNote();
		}
		?>
		...
		// exclude selected columns from all available columns
	...
	<table id="BASKET_TABLE" cellpadding="3" cellspacing="1" border="0" width="100%" class="internal">
		...
		<!-- table header WITHOUT stores columns -->
		...
		foreach ($arBasketItem as $val)
		{
			...
			$val["QUANTITY"] = $QUANTITY_FACTORIAL == 'Y' ? doubleval($val["QUANTITY"]) : intval($val["QUANTITY"]);
	...
	<td valign="top" align="left" colspan="2">
		...
		<div style="float:right">
			<script type="text/javascript">
				...
				{
					var quantity = 1,
						BUYER_ID = document.order_edit_info_form.user_id.value,
						BUYER_COUPONS = document.getElementById("COUPON").value,
						strUserColumns = BX("userColumns").value,
						caller = 'order_new';

					window.open('/bitrix/admin/sale_product_search.php?lang=<?=LANGUAGE_ID?>&LID=<?=CUtil::JSEscape($LID)?>&addDefault=N&caller=' + caller + '&func_name=getParamsByProductId&index=' + index + '&QUANTITY=' + quantity + '&BUYER_ID=' + BUYER_ID + '&BUYER_COUPONS=' + BUYER_COUPONS + '&ID=<?=CUtil::JSEscape($ID)?>&userColumns=' + strUserColumns, '', 'scrollbars=yes,resizable=yes,width=980,height=550,top='+parseInt((screen.height - 500)/2-14)+',left='+parseInt((screen.width - 840)/2-5));
				}
			</script>
			<?
...
<script type="text/javascript">
	...
	function FillProductFields(index, arParams, iblockID)
	{
		...
		for (var n = 0; n < columnsData.length; n++) // iterate over each column from the user's column set
		{
			...
			if ((columnName == "COLUMN_NUMBER") || (columnsData.indexOf("COLUMN_NUMBER") == -1 && necessaryColumns.indexOf("COLUMN_NUMBER") != -1))
			{
				var oCellNumber = oRow.insertCell(-1);
				oCellNumber.setAttribute('class','COLUMN_NUMBER');

				if (columnsData.indexOf("COLUMN_NUMBER") == -1)
				{
					oCellNumber.setAttribute('style', 'display:none');
					necessaryColumns.splice(necessaryColumns.indexOf("COLUMN_NUMBER"),1);
				}
			}

			if ((columnName == "COLUMN_IMAGE") || (columnsData.indexOf("COLUMN_IMAGE") == -1 && necessaryColumns.indexOf("COLUMN_IMAGE") != -1))
			{
				var oCellPhoto = oRow.insertCell(-1);
				oCellPhoto.setAttribute('class','COLUMN_IMAGE');

				if (columnsData.indexOf("COLUMN_IMAGE") == -1)
				{
					oCellPhoto.setAttribute('style', 'display:none');
					necessaryColumns.splice(necessaryColumns.indexOf("COLUMN_IMAGE"),1);
				}
			}
			...
			if ((columnName == "COLUMN_REMAINING_QUANTITY") || (columnsData.indexOf("COLUMN_REMAINING_QUANTITY") == -1 && necessaryColumns.indexOf("COLUMN_REMAINING_QUANTITY") != -1))
			{
				var oCellBalance = oRow.insertCell(-1);
				oCellBalance.setAttribute('class','COLUMN_REMAINING_QUANTITY');

				if (columnsData.indexOf("COLUMN_REMAINING_QUANTITY") == -1)
				{
					oCellBalance.setAttribute('style', 'display:none');
					necessaryColumns.splice(necessaryColumns.indexOf("COLUMN_REMAINING_QUANTITY"),1);
				}
			}

			if ((columnName == "COLUMN_PROPS") || (columnsData.indexOf("COLUMN_PROPS") == -1 && necessaryColumns.indexOf("COLUMN_PROPS") != -1))
			{
				var oCellPROPS = oRow.insertCell(-1);
				oCellPROPS.setAttribute('class','COLUMN_PROPS');

				if (columnsData.indexOf("COLUMN_PROPS") == -1)
				{
					oCellPROPS.setAttribute('style', 'display:none');
					necessaryColumns.splice(necessaryColumns.indexOf("COLUMN_PROPS"),1);
				}
			}
	...
	function getParamsByProductId(index, arParams, iblockID)
	{
	...
	{
		var obCartFix = BX('CART_FIX');
		var obCoupon = BX('COUPON');
		if (!!obCartFix && 'Y' == obCartFix.value)
		{
			if (!!obCoupon && 0 < obCoupon.value.length)
			{
				if (!BX('PAYED') || !BX('PAYED').checked)
					obCartFix.value = 'N';
			}
	...
	function fRecalProduct(id, type, recommendet, recalcAll)
	{
		...
		{
			var prevQuantity = parseFloat(BX('PRODUCT[' + id + '][QUANTITY]').defaultValue),
		...
		if (result["status"] == "Y")
		{
			...
			for (var i = 2, row; row = newTable.rows[i]; i++)
			{
				...
				for (var n = 0, column2add; column2add = arNecessaryColumns[n]; n++)
				{
					for (var p = 0, col; col = oldRow.cells[p]; p++)
					{
						if (!(column2add in columnsData) && (addedColumns.indexOf(column2add) == -1) && (col.className == column2add))
						{
							var cell = oldRow.cells[p];
				...
				<td valign="top" id="itog_tabs" class="load_product">
					...
					<div id="tabs">
						...
						$dbViewsList = CSaleViewedProduct::GetList(
								...
								false,
								array('nTopCount' => 10),
								array('ID', 'PRODUCT_ID', 'LID', 'MODULE', 'NAME', 'DETAIL_PAGE_URL', 'PRICE', 'CURRENCY', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'PRODUCT_PROVIDER_CLASS')
							);
						while ($arViews = $dbViewsList->Fetch())
							$arViewed[] = $arViews;
				...
				<td valign="top" class="summary">
					...
					<td nowrap>
						<div id="ORDER_WAIGHT" style="white-space:nowrap;">
							<?=roundEx(DoubleVal($productWeight/$WEIGHT_KOEF), SALE_WEIGHT_PRECISION)." ".$WEIGHT_UNIT;?>
						</div>
					</td>
					</tr>
					<tr>
...
if (isset($_GET["user_id"]) && isset($_GET["LID"]) && !$bVarsFromForm)
{
	...
	if ($bUseCatalog && $bUseIblock && isset($_GET["product"]) && !empty($_GET["product"]) && is_array($_GET["product"]))
	{
		$arProductId = array();
		$arGetProduct = array();
