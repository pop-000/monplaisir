...
if ((strlen($save)>0 || strlen($apply)>0) && $_SERVER['REQUEST_METHOD']=="POST" && $saleModulePermissions=="W" && check_bitrix_sessid())
{
	...
	if (strlen($save)>0 && strlen($strError)<=0)
		LocalRedirect("sale_delivery.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
}
...
$aTabs = array(
	...
	array("DIV" => "edit2", "TAB" => GetMessage("SDEN_TAB_PAYSYSTEM"), "ICON" => "sale", "TITLE" => GetMessage("SDEN_TAB_PAYSYSTEM_DESCR"))
);
