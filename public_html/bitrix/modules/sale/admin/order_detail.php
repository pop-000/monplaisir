					...
					foreach ($arOrderPropsValue as $arOrderProps)
					{
						...
						<tr>
							...
							elseif ($arOrderProps["TYPE"] == "LOCATION")
							{
								...
								elseif (strlen($locationString) <= 0)
									$locationString = $arVal["REGION_NAME"];
						...
						<td valign="top">
							...
							<script type="text/javascript">
								...
								function fChangeMarkResult(res)
								{
									...
									// 	BX('btn_show_mark').style.display = "none";
									// 	BX('btn_cancel_mark').style.display = "table-row";

									// 	if (rs["DATE_MARKED"] && rs["DATE_MARKED"].length > 0)
									// 		emp_mark_user = rs["DATE_MARKED"];

									// 	if (rs["EMP_MARKED_ID"] && rs["EMP_MARKED_ID"].length > 0)
									// 		emp_mark_user += ' ' + rs["EMP_MARKED_ID"];

									// 	if (BX('date_change_mark_user') && emp_mark_user.length > 0)
									// 		BX('date_change_mark_user').innerHTML = emp_mark_user;

									// 	BX('date_mark_cancel').style.display = "table-row";
									// 	BX('reason_mark_text').innerHTML = BX('FORM_REASON_MARKED').value;
									// 	BX('reason_mark').style.display = "table-row";
									// }
								}
									...
									if ($columnCode == "COLUMN_NAME")
									{
										if ($bUseIblock)
										{
											...
											if ($arProductInfo["IBLOCK_ID"] > 0)
												$arItem["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$arItem["PRODUCT_ID"]."&type=".$arProductInfo["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arProductInfo["IBLOCK_ID"]."&find_section_section=".IntVal($arProductInfo["IBLOCK_SECTION_ID"]);
										}
										?>
