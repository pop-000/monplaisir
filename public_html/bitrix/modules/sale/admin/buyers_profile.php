...
if(!empty($arUser))
{
	...
	// get quantity data for all catalog products
	if (!empty($arCatalogProductId))
	{
		if (CModule::IncludeModule('catalog'))
		{
			$dbElements = CCatalogProduct::GetList(array(), array("ID" => $arCatalogProductId), false, false, array("ID", "QUANTITY"));
			while ($arElement = $dbElements->GetNext())
			{
				foreach ($arViewsData as &$arViews)
				{
					if ($arViews["PRODUCT_ID"] == $arElement["ID"])
						$arViews["QUANTITY"] = $arElement["QUANTITY"];
				}
				unset($arViews);
			}
		}
	...
	foreach ($arViewsData as $arViews)
	{
		...
		if (count($arResult["SKU_ELEMENTS"]) > 0):
			...
			$linkBasket = 'fAddToBasketViewed('.$arViews["PRODUCT_ID"].', \''.$arViews["LID"].'\')';
		endif;
