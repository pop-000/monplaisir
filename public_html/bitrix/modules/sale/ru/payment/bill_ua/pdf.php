...
if ($arBasket = $dbBasket->Fetch())
{
	...
	do
	{
		...
		if ($productName == "OrderDelivery")
			...
			$productName = htmlspecialcharsbx("Знижка");
		...
		$arCells[++$n] = array(
			...
			CSalePdf::prepareToPdf(CurrencyFormatNumber($arBasket["PRICE"], $arBasket["CURRENCY"])),
			CSalePdf::prepareToPdf(roundEx($arBasket["VAT_RATE"]*100, SALE_VALUE_PRECISION)."%"),
			CSalePdf::prepareToPdf(CurrencyFormatNumber(
...
*/
	...
	$arColsCaption = array(
		...
		CSalePdf::prepareToPdf($vat <= 0 ? 'Ціна без ПДВ' : 'Ціна з ПДВ'),
		CSalePdf::prepareToPdf('Ставка ПДВ'),
		CSalePdf::prepareToPdf($vat <= 0 ? 'Сума без ПДВ' : 'Сума з ПДВ')
	);
	for ($i = 1; $i <= 7; $i++)
