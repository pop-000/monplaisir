...
$arOrder = false;
if ($bUseAccountNumber) // supporting order ACCOUNT_NUMBER or ID in the URL
{
	$arFilter = array(
		"USER_ID" => $USER->GetID(),
		"ACCOUNT_NUMBER" => $ID,
...
if ($arOrder)
{
	if($arParams["SET_TITLE"] == 'Y')
		$APPLICATION->SetTitle(str_replace("#ID#", $arOrder["ACCOUNT_NUMBER"], GetMessage("SPOD_TITLE")));
	...
	if ($arResult["CAN_CANCEL"] == "Y")
		$arResult["URL_TO_CANCEL"] = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_CANCEL"], Array("ID" => urlencode(urlencode($arOrder["ACCOUNT_NUMBER"])))).'CANCEL=Y';
	...
	if($arUser = $dbUser->GetNext())
	{
		...
		$arResult["USER_NAME"] = CUser::FormatName(CSite::GetNameFormat(false), $arUser, true, false);
	}
	...
	$arResult["TRACKING_NUMBER"] = $arOrder["TRACKING_NUMBER"];
	...
	$dbBasket = CSaleBasket::GetList(
			...
			array("ID", "DETAIL_PAGE_URL", "NAME", "NOTES", "QUANTITY", "PRICE",
				...
				"VAT_RATE", "PRODUCT_XML_ID", "TYPE", "SET_PARENT_ID"
			)
		);
	...
	while ($arBasket = $dbBasket->Fetch())
	{
		...
		$arBasketTmp["WEIGHT_FORMATED"] = roundEx(DoubleVal($arBasketTmp["WEIGHT"]/$arResult["WEIGHT_KOEF"]), SALE_WEIGHT_PRECISION)." ".$arResult["WEIGHT_UNIT"];
