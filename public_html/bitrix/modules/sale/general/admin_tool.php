		...
		if(!empty($arProductId))
		{
			...
			foreach ($arProducts as $arProduct)
			{
				...
				if ($arProduct["IBLOCK_ID"] > 0)
					$arProduct["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$arProduct["PRODUCT_ID"]."&type=".$arProduct["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$arProduct["IBLOCK_ID"]."&find_section_section=".intval($arProduct["IBLOCK_SECTION_ID"]);
				...
				if (!empty($arSet) && array_key_exists($arProduct["PRODUCT_ID"], $arSet))
				{
					...
					{
						$editUrl = "/bitrix/admin/iblock_element_edit.php?ID=".$set["ITEM_ID"]."&type=".$set["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$set["IBLOCK_ID"]."&find_section_section=".intval($set["IBLOCK_SECTION_ID"]);
 ...
 *
 * Currently is used to show order properties related to payment/delivery systems
 */
...
/*
 * Returns HTML control with payment system data
 */
...
/*
 * Returns HTML select control with delivery service data for admin pages
 */
...
/*
 * get HTML for recommended product, basket product or product from the viewed list
 */
	...
	if (CModule::IncludeModule('catalog') && CModule::IncludeModule('iblock'))
	{
		...
		if(!empty($arProductId))
		{
			...
			while($arProduct = $dbProduct->Fetch())
			{
				...
				$ar_res = CCatalogProduct::GetByID($item["PRODUCT_ID"]);
...
/*
 * Returns array of product parameters to fill basket table in the order_new.php file
...
function getProductDataToFillBasket($productId, $quantity, $userId, $LID, $userColumns, $tmpId = "")
{
	...
	$arUserColumns = (strlen($userColumns) > 0) ? explode(",", $userColumns) : array();
	...
	if (count($arProductData) > 0)
	{
		...
		$arElementInfo["NAME"] = htmlspecialcharsex($arElementInfo["NAME"]);
		$arElementInfo["DETAIL_PAGE_URL"] = htmlspecialcharsex($arElementInfo["DETAIL_PAGE_URL"]);

		if ($arElementInfo["IBLOCK_ID"] > 0)
			$arElementInfo["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$arElementInfo["PRODUCT_ID"]."&type=catalog&lang=".LANG."&IBLOCK_ID=".$arElementInfo["IBLOCK_ID"]."&find_section_section=".$arElementInfo["IBLOCK_SECTION_ID"];
		...
		if(doubleval($ar_res["WEIGHT"]) > 0)
			$arElementInfo["WEIGHT"] = $ar_res["WEIGHT"];

		// measure
		$arElementInfo["MEASURE_TEXT"] = "";
		if (intval($ar_res["MEASURE"]) > 0)
		{
			$dbMeasure = CCatalogMeasure::GetList(array(), array("ID" => intval($ar_res["MEASURE"])), false, false, array("ID", "SYMBOL_RUS", "SYMBOL_INTL"));
			if ($arMeasure = $dbMeasure->Fetch())
				$arElementInfo["MEASURE_TEXT"] = (LANGUAGE_ID == "ru") ? $arMeasure["SYMBOL_RUS"] : $arMeasure["SYMBOL_INTL"];
		}

		// ratio
		$arElementInfo["RATIO"] = 1;
		$dbratio = CCatalogMeasureRatio::GetList(array(), array("PRODUCT_ID" => $productId));
		if ($arRatio = $dbratio->GetNext())
		...
		if ($productProvider = CSaleBasket::GetProductProvider(array("MODULE" => $arElementInfo["MODULE"], "PRODUCT_PROVIDER_CLASS" => $arElementInfo["PRODUCT_PROVIDER_CLASS"])))
		{
			...
			if ($ar_res["TYPE"] == CCatalogProduct::TYPE_SET)
			{
				if (method_exists($productProvider, "GetSetItems"))
				{
		...
		$arParams["priceDiscount"] = floatval($arElement["DISCOUNT_PRICE"]);
		...
		$arParams["summaFormated"] = CurrencyFormatNumber($arElement["PRICE"], $arElementInfo["CURRENCY"]);
		...
		$arParams["barcodeMulti"] = $ar_res["BARCODE_MULTI"];
...
/*
 * Is used to re-sort basket items data so set parents will be added before set items
 */
function cmpSetData($arBasketItemA, $arBasketItemB)
{
