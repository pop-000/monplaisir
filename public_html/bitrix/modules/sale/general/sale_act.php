...
class CSaleDiscountActionApply
{
	...
	{
		if (array_key_exists('PRICE_DELIVERY', $arOrder))
		{
			...
			if (0 <= $dblResult)
			{
				if (!array_key_exists('PRICE_DELIVERY_DIFF', $arOrder))
	...
	public static function Generate($arOneCondition, $arParams, $arControl, $arSubs = false)
	{
		...
		if (!$boolError)
		{
			...
			else
			{
				...
				$strResult = 'array_key_exists(\''.$arControl['FIELD'].'\', '.$arParams['BASKET_ROW'].') && '.$strResult;
			}
		}
...
class CSaleActionTree extends CGlobalCondTree
{
	...
	public function GenerateLevel(&$arLevel, $arParams, $boolFirst = false)
	{
		...
		if ($boolFirst)
		{
			...
			if (array_key_exists('CLASS_ID', $arLevel) && !empty($arLevel['CLASS_ID']))
			{
				if (array_key_exists($arLevel['CLASS_ID'], $this->arControlList))
				{
		...
		else
		{
			...
			{
				$arParams['ROW_NUM'] = $intRowNum;
				if (array_key_exists('CLASS_ID', $arOneCondition) && !empty($arOneCondition['CLASS_ID']))
				{
					if (array_key_exists($arOneCondition['CLASS_ID'], $this->arControlList))
					{
