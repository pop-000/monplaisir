...
class CSaleHelper
{
	...
	*/
	...
	public static function getAdminHtml($fieldId, $arField, $fieldName, $formName)
	{
		...
		switch ($arField["TYPE"])
		{
			...
			case "CHECKBOX":
				...
				$resultHtml .= '<input '.
									'type="checkbox" '.
									'name="'.$fieldName.'['.htmlspecialcharsbx($fieldId).']" '.
									'value="Y" '.
				...
				if(isset($arField['HIDE_BY_NAMES']) && is_array($arField['HIDE_BY_NAMES']))
				{
					...
					<script language="JavaScript">
						BX.ready(
							function(){
								var cbObj = document.forms["'.$formName.'"]["'.$fieldName.'['.htmlspecialcharsbx($fieldId).']"];
			...
			case "DROPDOWN":
				...
				$resultHtml .= '<select name="'.$fieldName.'['.htmlspecialcharsbx($fieldId).']"';
			...
			case "MULTISELECT":
				$resultHtml .= '<select name="'.$fieldName.'['.htmlspecialcharsbx($fieldId).']" multiple="multiple">';
	...
	private static function getShopLocationParams($siteId = false)
	{
		...
		if(intval($locId) <= 0 && strlen($locZip) <= 0)
		{
			$locId = 0;
			$locZip = "0";
			...
			if ($loc_diff == "Y" && ($siteId !== false || defined(SITE_ID)))
			{
				...
				$locId = COption::GetOptionInt('sale', 'location', '', SITE_ID);
				$locZip = COption::GetOptionString('sale', 'location_zip', '', SITE_ID);
			}
			else
			{
	...
	public static function getShopLocationId()
	{
		static $shopLocationId = 0;
		...
		if(intval($shopLocationId) <= 0)
		{
			...
			if(isset($locParams['ID']) && intval($locParams['ID']) > 0)
				$shopLocationId = intval($locParams['ID']);
		}
	...
	public static function getShopLocation()
	{
		...
		{
			$shopLocationId = self::getShopLocationId();
