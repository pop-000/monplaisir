...
if ($REQUEST_METHOD == "POST" && strlen($Update) > 0 && $SALE_RIGHT == "W" && check_bitrix_sessid())
{
	...
	else
	{
		...
		$prevVal = COption::GetOptionString("sale", "product_reserve_clear_period", "0");
		if (intval($product_reserve_clear_period) != $prevVal)
		{
			CAgent::RemoveAgent("CSaleOrder::ClearProductReservedQuantity();", "sale");
			if (intval($product_reserve_clear_period) > 0)
				CAgent::AddAgent("CSaleOrder::ClearProductReservedQuantity();", "sale", "N", intval($product_reserve_clear_period)*24*60*60, "", "Y");
		}

		COption::SetOptionString("sale", "product_reserve_clear_period", intval($product_reserve_clear_period));
...
<?
	...
	<tr>
		...
		<td>
			<?
			$bEnableReservation = (COption::GetOptionString("catalog", "enable_reservation") != "N") ? true : false;
			...
			?>
			<select name="PRODUCT_RESERVE_CONDITION" id="sl_reserve" <?if (!$bEnableReservation) echo " disabled";?>>
				<?
		...
		<tr>
			<td>
				<?echo GetMessage("SALE_RESERVATION_CLEAR_PERIOD")?>
			</td>
			<td>
