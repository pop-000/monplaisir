...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/learning/admin/task_description.php");
...
if (isset($_POST['PARENT_LESSON_ID']))
{
	...
	{
		throw new LearnException (
			'EA_LOGIC: PARENT_LESSON_ID and/or PROPOSE_RETURN_LESSON_PATH and/or '
			. 'LESSON_PATH are mutually exclusive arguments.', 
			LearnException::EXC_ERR_ALL_LOGIC);
	}
	...
	if ( ! $was_errors )
	{
		if(strlen($apply) <= 0)
		{
			if (strlen($return_url) > 0)
				LocalRedirect(str_replace("#LESSON_ID#", $LESSON_ID, $return_url));
			else
			{
				if (isset($g_learn_parentLessonPath))
				...
				LocalRedirect("/bitrix/admin/learn_unilesson_admin.php?lang=" . LANG . $uriParentLessonPath . GetFilterParams("filter_", false));
			}
		}
		...
		if (isset($g_learn_currentLessonPath))
			...
			$uriCurrentLessonPath = '';	// was: '&LESSON_PATH=' . (int) $LESSON_ID;
...
elseif ($LESSON_ID == 0)
{
	if (isset($g_learn_parentLessonId))
	{
		if ($oAccess->IsLessonAccessible ($g_learn_parentLessonId, CLearnAccess::OP_LESSON_LINK_DESCENDANTS)
...
if (isset($g_learn_parentLessonId))
{
	...
	$arParentLessonData  = $resParentLessonData->Fetch();
...
$tabControl->BeginCustomField("PREVIEW_TEXT", GetMessage("LEARNING_PREVIEW_TEXT"), false);?>
	...
	<?endif?>
<?php $tabControl->EndCustomField("PREVIEW_TEXT", '<input type="hidden" id="PREVIEW_TEXT" name="PREVIEW_TEXT" value="' . $str_PREVIEW_TEXT . '">');?>
