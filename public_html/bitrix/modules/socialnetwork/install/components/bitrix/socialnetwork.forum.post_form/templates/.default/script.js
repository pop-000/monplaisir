...
function PostFormAjaxMsgStart(msg)
{
...
/*	if (formlist = BX.findChild(forumlist, {tagName: 'form', className: 'forum-form'}, true))
	...
	if (result.status)
	{
		...
		else if (!!result.previewMessage)
		{
			...
			previewDIV = fReplaceOrInsertNode(result.previewMessage, previewDIV, previewParent, {className : 'forum_post_form'});

			PostFormAjaxStatus('');
			fRunScripts(result.previewMessage);
			if (previewDIV)
				BX.scrollToNode(previewDIV);
		}
...
function ClearForumPostForm(form)
{
	...
	var filesForm = BX.findChild(form, {'className': 'wduf-placeholder-tbody' }, true, false);
	if(filesForm !== null && typeof filesForm != 'undefined')
		BX.cleanNode(filesForm, false);
	var filesForm = BX.findChild(form, {'className': 'wduf-selectdialog' }, true, false)
	if(filesForm !== null && typeof filesForm != 'undefined')
		BX.hide(filesForm);
...
function quoteMessageEx(author, mid)
{
	...
	if (selection != "")
	{
		...
		if (author != null && author)
			selection = author + (oText ? oText['author'] : '') + selection;
