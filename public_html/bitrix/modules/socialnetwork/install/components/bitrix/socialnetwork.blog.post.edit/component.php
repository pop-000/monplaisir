...
if (!IsModuleInstalled('intranet')) // Disable calendar feature for non cp
	$bCalendar = false;
if ($bCalendar && CModule::IncludeModule('extranet') && !CExtranet::IsIntranetUser()) // Disable calendar feature for extranet users
	$bCalendar = false;
	...
	)
	{
		...
		if($arPost = CBlogPost::GetByID($delId))
		{
			...
			if($perms >= BLOG_PERMS_FULL)
			{
				...
				BXClearCache(true, "/".SITE_ID.CComponentEngine::MakeComponentPath("bitrix:socialnetwork.blog.blog"));
		...
		else
		{
			foreach ($arResult["PostToShow"]["SPERM"] as $type => $ar)
			{
				if(is_array($ar))
				{
					...
					{
						if ($type == 'U' && $value == 'A' && $bDefaultToAll)
							$arResult["PostToShow"]["FEED_DESTINATION"]['SELECTED']['UA'] = 'groups';
						elseif ($type == 'U')
						{
