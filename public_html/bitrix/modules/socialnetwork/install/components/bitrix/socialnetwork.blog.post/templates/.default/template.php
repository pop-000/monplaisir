	...
	if(!empty($arResult["Post"])>0)
	{
		...
		<div class="<?=$className?>" id="blg-post-<?=$arResult["Post"]["ID"]?>">
			...
			<div class="feed-post-cont-wrap<?=$aditStyles?>" id="blg-post-img-<?=$arResult["Post"]["ID"]?>">
				<div class="feed-user-avatar"<?=(strlen($arResult["arUser"]["PERSONAL_PHOTO_resized"]["src"]) > 0 ? " style=\"background:#ffffff url('".$arResult["arUser"]["PERSONAL_PHOTO_resized"]["src"]."') no-repeat center;\"" : "")?>></div>
				<div class="feed-post-title-block"><?
					...
					?><a class="feed-post-user-name" id="bp_<?=$anchor_id?>" href="<?=$arResult["arUser"]["url"]?>" bx-post-author-id="<?=$arResult["arUser"]["ID"]?>"><?=CUser::FormatName($arParams["NAME_TEMPLATE"], $arTmpUser, ($arParams["SHOW_LOGIN"] != "N" ? true : false))?></a><?
				...
				?></div>
				<div class="feed-post-text-block<?=($arResult["Post"]["IS_IMPORTANT"] ? " feed-info-block" : "")?>">
						...
						if (!empty($arResult["Post"]["IMPORTANT"]))
						{
							?><div class="feed-imp-post-footer"><?
								...
								?><span <?
									...
									if($arResult["Post"]["IMPORTANT"]["COUNT"]<=0){?> style="display:none;"<?}?>><?=GetMessage("BLOG_USERS_ALREADY_READ")?> <a class="feed-imp-post-user-link" href="javascript:void(0);"><?
