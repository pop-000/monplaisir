...
{
	CUtil::InitJSCore(array("ajax", "window", "tooltip", "popup", "fx"));
		...
		if ($arParams["IS_CRM"] == "Y")
		{
			$GLOBALS['APPLICATION']->IncludeComponent(
				...
				array(
					...
					'ENABLE_CALENDAR_EVENT_ADD' => false,
	...
	)
	{
		...
		{
			?><a onclick="__logGetNextPage('<?=$next_url?>')" href="javascript:void(0);" id="sonet_log_more_container" class="feed-new-message-inf-bottom"><?
				?><span class="feed-new-message-inf-text"><?=GetMessage("SONET_C30_MORE")?><?
					?><span class="feed-new-message-icon"></span><?
				?></span><?
				?><span class="feed-new-message-inf-text feed-new-message-inf-text-waiting" style="display: none;"><?
					?><span class="feed-new-message-wait-icon"></span><?
					?><?=GetMessage("SONET_C30_T_MORE_WAIT")?><?
				?></span><?
			?></a><?
		}
	...
	if (!$arResult["AJAX_CALL"])
	{
		...
		{
			?><a style="display: none;" href="javascript:void(0);" id="sonet_log_more_container_first" class="feed-new-message-inf-bottom"><?
	...
	<form action="" id="<?=$arParams["FORM_ID"]?>" name="<?=$arParams["FORM_ID"]?>" <?
		...
		<?$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"UPLOAD_FILE" => (
					!empty($arResult["COMMENT_PROPERTIES"]["DATA"]["UF_SONET_COM_DOC"])
						? false
...
BX.ready(function(){
	...
	if (!!FCForm)
	{
		...
		BX.addCustomEvent(window, "OnBeforeSocialnetworkCommentShowedUp", function(entity){ if (entity != 'socialnetwork') { window["UC"]["f<?=$arParams["FORM_ID"]?>"].hide(true); } } );
