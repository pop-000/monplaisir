...
else
{
	...
	if($arResult['bEdit'] == 'Y' && $_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["submit"])>0 && check_bitrix_sessid())
	{
		...
		if (
			...
			&& (
				!isset($_POST['PERSONAL_PHOTO_CID'])
				|| (
					is_array($_SESSION["MFI_UPLOADED_FILES_".$_POST['PERSONAL_PHOTO_CID']])
					&& in_array($_POST['PERSONAL_PHOTO_ID'], $_SESSION["MFI_UPLOADED_FILES_".$_POST['PERSONAL_PHOTO_CID']])
				)
			)
		)
		{
		...
		{
			if (strlen($_FILES["PERSONAL_PHOTO"]["name"]) > 0 || isset($_POST["PERSONAL_PHOTO_del"]))
		...
		//WORK_LOGO upload
		if (strlen($_FILES["WORK_LOGO"]["name"]) > 0 || isset($_POST["WORK_LOGO_del"]))
		{
