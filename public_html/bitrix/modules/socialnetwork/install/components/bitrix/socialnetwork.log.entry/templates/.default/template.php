	...
	)
	{
		...
		?><div class="feed-post-block<?=($is_unread ? " feed-post-block-new" : "")?><?=(array_key_exists("EVENT_FORMATTED", $arEvent) && array_key_exists("STYLE", $arEvent["EVENT_FORMATTED"]) && strlen($arEvent["EVENT_FORMATTED"]["STYLE"]) > 0 ? " feed-".$arEvent["EVENT_FORMATTED"]["STYLE"] : "")?>">
			...
			?>"><?
				...
				?><div class="feed-user-avatar"<?=(strlen($arEvent["AVATAR_SRC"]) > 0 ? " style=\"background:#ffffff url('".$arEvent["AVATAR_SRC"]."') no-repeat center;\"" : "")?>></div>
...
//					"blog_post" => array("BLOG", "BG"),
				...
				if (array_key_exists($arEvent["EVENT"]["EVENT_ID"], $entities_xml_id) && $arEvent["EVENT"]["SOURCE_ID"] > 0)
				{
					$entity_type = $entities_xml_id[$arEvent["EVENT"]["EVENT_ID"]][1];
					$entity_xml_id = $entities_xml_id[$arEvent["EVENT"]["EVENT_ID"]][0]."_".$arEvent["EVENT"]["SOURCE_ID"];
					$notifyTags = $entities_xml_id[$arEvent["EVENT"]["EVENT_ID"]][2];
				}
				else
				{
					$entity_xml_id = strtoupper($arEvent["EVENT"]["EVENT_ID"])."_".$arEvent["EVENT"]["ID"];
					$entity_type = substr($entity_xml_id, 0, 2);
					$notifyTags = "";
				}
				$tplID = 'SOCCOMMENT_'.$entity_xml_id.'_';
				...
				?><div class="feed-post-informers"><?
					...
					)
					{
						...
						?><span class="feed-inform-comments"><?
							?><?if (intval($arEvent["COMMENTS_COUNT"]) > 0 ) { ?><a href="<?=$url?>"><?=GetMessage("SONET_C30_COMMENTS")?></a><? }
							else { ?><a href="javascript:void(0);" onclick="BX('feed_comments_block_<?=$arEvent["EVENT"]["ID"]?>').style.display = 'block';<?
								?>__logShowCommentForm('<?=$entity_xml_id?>')"><?=GetMessage("SONET_C30_COMMENT_ADD")?></a><? }
						?></span><?
					}
					else
						$bHasComments = false;
			...
			)
			{
				...
				$arRecords = array();
				if (!!$component && !!$component->__parent)
				{
					if (!!$component->__parent->arResult)
					{
						...
						$component->__parent->arResult["ENTITIES_XML_ID"][$entity_xml_id] = array($entity_type, $arEvent["EVENT"]["SOURCE_ID"]);
						...
						$component->__parent->arResult["ENTITIES_CORRESPONDENCE"][$entity_xml_id."-0"] = array($arEvent["EVENT"]["ID"], 0);
					}
				}
				if (!empty($arEvent["COMMENTS"]))
				{
					foreach($arEvent["COMMENTS"] as $arComment)
					{
						...
						$component->__parent->arResult["ENTITIES_CORRESPONDENCE"][$entity_xml_id."-".$commentId] =
				...
				$arResult["OUTPUT_LIST"] = $APPLICATION->IncludeComponent(
					...
					array(
						...
						"SHOW_POST_FORM" => (isset($arEvent["HAS_COMMENTS"]) && $arEvent["HAS_COMMENTS"] === "Y" && isset($arEvent["CAN_ADD_COMMENTS"])
							&& $arEvent["CAN_ADD_COMMENTS"] === "Y" ? "Y" : "N"),
						"SHOW_MINIMIZED" => "Y",
				...
				if ($diff > 0)
				{
					...
					?><div class="feed-com-header"><?
						?><a class="feed-com-all" <?
							...
							?>id="<?=$entity_xml_id?>_page_nav"><?
							?><?=GetMessage("SONET_C30_PREV_COMMENTS")?> (<?=$diff?>)<i></i></a><?
					?></div><?
					?><div id="<?=$entity_xml_id?>_hidden_records" style="display:none; overflow:hidden;"></div><?
				}
					?><?=$arResult["OUTPUT_LIST"]["HTML"]?><?
					?><script type="text/javascript">BX.ready(function(){ __logCommentsListRedefine("<?=$entity_xml_id?>", "sonet_log_day_item_<?=$ind?>", "anchor_<?=CUtil::JSEscape($anchor_id)?>");});</script><?
