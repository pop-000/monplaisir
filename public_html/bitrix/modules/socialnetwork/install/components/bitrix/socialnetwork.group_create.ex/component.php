		...
		if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["save"]) > 0 && check_bitrix_sessid())
		{
			...
			if (!array_key_exists("TAB", $arResult) || $arResult["TAB"] == "edit")
			{
				if (
					...
					&& (
						!isset($_POST['GROUP_IMAGE_CID'])
						|| (
							is_array($_SESSION["MFI_UPLOADED_FILES_".$_POST['GROUP_IMAGE_CID']])
							&& in_array($_POST['GROUP_IMAGE_ID'], $_SESSION["MFI_UPLOADED_FILES_".$_POST['GROUP_IMAGE_CID']])
						)
					)
				)
				{
					...
					if (CModule::IncludeModule('extranet') && strlen($_POST["EMAILS"]) > 0)
					{
						...
						if (count($arEmail) > 0)
						{
							...
							foreach($arEmail as $email)
							{
								...
								$rsUser = CUser::GetList(($by="id"), ($order="asc"), $arFilter, array("SELECT" => array("UF_DEPARTMENT", "LAST_LOGIN", "LAST_ACTIVITY_DATE")));
								if ($arUser = $rsUser->Fetch())
								{
									...
									if ((sizeof($arUser["UF_DEPARTMENT"]) == 0) && IsModuleInstalled("extranet"))
									{
										...
										//extranet invitation already sent, reinvite again
										if (strlen($checkword) > 8)
										{
											$user = new CUser;
											$arFields = array(
								...
								if ($userID > 0 && (empty($arUser) || (strlen($arUser["LAST_LOGIN"]) <= 0 && strlen($arUser["LAST_ACTIVITY_DATE"]) <= 0)))
								{
									...
									if ($rel != SONET_ROLES_REQUEST && $rel != SONET_ROLES_BAN)
									{
										$event = new CEvent;
										$arFields = Array(
											"USER_ID" => $userID,
					...
					if (is_array($arUserIDs) && count($arUserIDs) > 0)
					{
						foreach($arUserIDs as $user_id)
						{
							...
							if (!$isCurrentUserTmp && $canInviteGroup && !$user2groupRelation)
							{
								...
								if (!CSocNetUserToGroup::SendRequestToJoinGroup($GLOBALS["USER"]->GetID(), $user_id, $arResult["GROUP_ID"], $_POST["MESSAGE"], $bMail))
								{
							...
							else if (!$isCurrentUserTmp && $canInviteGroup && $user2groupRelation)
							{
								$rsUser = CUser::GetByID($user_id);
								...
								if (!in_array("USERS", $arResult["ErrorFields"]))
									$arResult["ErrorFields"][] = "USERS";
							}
						}
					}
