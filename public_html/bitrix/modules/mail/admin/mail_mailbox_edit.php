...
<?$tabControl->BeginNextTab();?>
	...
	<tr>
		...
		<td width="60%">
			<select id="mailbox_site_id" name="LID"<? if ($str_SERVER_TYPE == 'imap') { ?> onchange="changeServicesList();"<? } ?>>
			<? $defSite = false; ?>
			<? $result = Bitrix\Main\SiteTable::getList(array('order' => array('SORT' => 'ASC'))); ?>
			<? while (($site = $result->fetch()) !== false) { ?>
				<? $defSite = $str_LID == $site['LID'] ? $site['LID'] : ($defSite ?: $site['LID']); ?>
				<option value="<?=$site['LID']; ?>"
					<? if ($str_SERVER_TYPE == 'imap' && empty($mailServices[$site['LID']])) { ?>disabled="disabled"<? } ?>
					<? if ($str_LID == $site['LID']) { ?>selected="selected"<? } ?>>
					<?=$site['NAME']; ?><? if ($str_SERVER_TYPE == 'imap' && empty($mailServices[$site['LID']])) { ?> *<? } ?>
				</option>
			<? } ?>
			</select>
		</td>
	...
	<tr>
		...
		<td>
			...
			<div id="el1" class="imap"><?=GetMessage('MAIL_MBOX_SERVER_TYPE_IMAP_DESC'); ?></div>
			<div id="el2" class="smtp"><?=GetMessage('MAIL_MBOX_SERVER_TYPE_SMTP_DESC'); ?><br><?=GetMessage('MAIL_MBOX_SERVER_TYPE_SMTP_A'); ?></div>
		</td>
	</tr>
	<? } ?>
	<tr class="adm-detail-required-field">
	...
	<tr id="el9" class="pop3 imap adm-detail-required-field">
	...
	<tr id="el12" class="imap adm-detail-required-field">
...
<? } ?>
	...
	<? if ($str_SERVER_TYPE == 'imap') { ?>
	<input type="hidden" value="imap" name="SERVER_TYPE" id="SERVER_TYPE">
	<? } ?>
