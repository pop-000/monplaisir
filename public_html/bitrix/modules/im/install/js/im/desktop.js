...
;(function (window)
{
	...
	BX.desktop.prototype.init = function ()
	{
		...
		BX.addCustomEvent("onPullError", BX.delegate(function(error) {
			...
			{
				this.openConfirm('<div class="bx-desktop-reconnect">'+BX.message('BXD_RECONNECT')+'</div>', false);
	...
	BX.desktop.prototype.notSupported = function ()
	{
		...
		var updateContent = BX.create("div", { props : { className : "bx-desktop-update-box" }, children : [
			...
			BX.create("div", { props : { className : "bx-desktop-update-box-btn" }, events : { click :  BX.delegate(this.checkUpdate, this)}, html: '<span class="bx-desktop-button bx-desktop-button-green"><i class="bx-desktop-button-fc"></i><span>'+BX.message('BXD_NEED_UPDATE_BTN')+'</span><i></i></span>'})
		]});
		BX.ready(function(){
	...
	BX.desktop.prototype.logout = function (terminate)
	{
		if (!this.ready()) return false;
	...
	BX.desktop.prototype.checkUpdate = function ()
	{
		if (!this.ready()) return true;

		if (this.enableInVersion(16))
	...
	{
		if (!this.ready() && !this.enableInVersion(20))
			return false;
	...
	BX.desktop.prototype.adjustSize = function (width, height)
	{
		...
		if ((!width || !height) && (window.innerHeight < this.minHeight || window.innerWidth < this.minWidth))
		{
	...
	BX.desktop.prototype.windowCommand = function (windowTarget, command)
	{
		...
		{
			console.log('ExecuteCommand Error', command, e);
			console.trace();
		}
	...
	BX.desktop.prototype.closeTopmostWindow = function()
	{
		...
		{
			this.windowCommand("close", this.topmostWindow);
			this.topmostWindow = null;
		}
		return true;
	}
	...
	{
		if (!this.ready() || !this.enableInVersion(22)) return false;
	...
	BX.desktop.prototype.drawTab = function (params)
	{
		...
		{
			this.contentTab.appendChild(
				BX.create('div', { attrs : { 'data-id' : params.id, id: 'bx-desktop-tab-'+params.id, title: params.title}, props : { className : "bx-desktop-tab bx-desktop-tab-"+params.id+(this.currentTab == params.id? ' bx-desktop-tab-active': '') }, children: [
					BX.create('span', { props : { className : "bx-desktop-tab-counter" }, html: params.counter? '<span class="bx-desktop-tab-counter-digit">'+params.counter+'</span>': ''}),
	...
	BX.desktop.prototype.drawAppearance = function ()
	{
		...
		this.content.appendChild(
			BX.create("div", { props : { className : 'bx-desktop-appearance'}, children: [
	...
	{
		if (!this.tabItems[tabId] || !BX('bx-desktop-tab-'+tabId))
			return false;
		...
		this.tabItems[tabId].badge = value;
	...
	{
		if (!this.tabItems[tabId] || !BX.type.isDomNode(content))
			return false;
	...
	BX.desktop.prototype.setUserInfo = function (params)
	{
		...
		this.contentAvatar.appendChild(
			BX.create('a', { attrs : { href : this.userInfo.profile, title : BX.util.htmlspecialcharsback(this.userInfo.name) }, props : { className : "bx-desktop-avatar" }, children: [
				BX.create('img', { attrs : { src : this.userInfo.avatar}, props : { className : "bx-desktop-avatar-img" }})
			]})
		);
