...
/*. void .*/
	public static function startCaching()
	{
		...
		if (
			$useCompositeCache
			&& array_key_exists("actionType", $_GET)
		)
		{
			//"actionType", "CACHE_MODE"
			define("actionType", $_GET["actionType"]);
			unset($_GET["actionType"]);
			unset($_REQUEST["actionType"]);

			define("CACHE_MODE", $_GET["CACHE_MODE"]);
			unset($_GET["CACHE_MODE"]);
			unset($_REQUEST["CACHE_MODE"]);

			if (isset($_SERVER["REQUEST_URI"]))
				$_SERVER["REQUEST_URI"] = preg_replace("/([?&]actionType=get_dynamic.*)\$/", "", $_SERVER["REQUEST_URI"]);
			file_put_contents("/tmp/debug02.log", $_SERVER["REQUEST_URI"], FILE_APPEND);
			if (isset($_SERVER["QUERY_STRING"]))
				$_SERVER["QUERY_STRING"] = preg_replace("/([?&]actionType=get_dynamic.*)\$/", "", $_SERVER["QUERY_STRING"]);
		}
		...
		if(file_exists($PAGES_FILE))
		{
			...
			if($fp !== false)
			{
				...
				if ($useCompositeCache)
				{
					...
					$values = array(
						md5($arHTMLPagesOptions["SERVER_UNIQ_ID"].$sess_id),
					);

					$contents = str_replace($patterns, $values, $contents);
				}
...
/*. float .*/
	...
	public static function CompileOptions(&$arOptions)
	{
		...
		$arOptions["SERVER_UNIQ_ID"] = CMain::GetServerUniqID();
	}
