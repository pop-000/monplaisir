...
class MssqlSqlHelper extends SqlHelper
{
	...
	public function getDateTimeToDbFunction(Type\DateTime $value, $type = Type\DateTime::DATE_WITH_TIME)
	{
		$customOffset = $value->getOffset();

		$serverTime = new Type\DateTime();
		...
		$format = ($type == Type\DateTime::DATE_WITHOUT_TIME ? "Y-m-d" : "Y-m-d H:i:s");
		...
		switch ($columnInfo["TYPE"])
		{
			...
			case "timestamp":
				if (empty($value))
					...
					$result = $this->getDatetimeToDbFunction($value, Type\DateTime::DATE_WITH_TIME);
				break;
			case "date":
				if (empty($value))
					$result = "NULL";
				else
					$result = $this->getDatetimeToDbFunction($value, Type\DateTime::DATE_WITHOUT_TIME);
				break;
			case "int":
			case "tinyint":
			case "smallint":
