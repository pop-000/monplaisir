...
;(function(window){
	...
	BX.PULL.getChannelID = function(code, withoutCache, send)
	{
		...
		BX.ajax({
			url: '/bitrix/components/bitrix/pull.request/ajax.php?GET_CHANNEL&V='+_revision+'&CR='+_channelClearReason+'&CODE='+code.toUpperCase()+(_mobileMode? '&MOBILE':''),
			method: 'POST',
			dataType: 'json',
	...
	BX.PULL.updateWatch = function(force)
	{
		...
		_watchTimeout = setTimeout(function()
		{
			...
			{
				BX.ajax({
					url: '/bitrix/components/bitrix/pull.request/ajax.php?UPDATE_WATCH&V='+_revision+'',
					method: 'POST',
					dataType: 'json',
					timeout: 30,
