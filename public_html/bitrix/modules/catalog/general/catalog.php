	...
	public function PrepareSql(&$arFields, $arOrder, $arFilter, $arGroupBy, $arSelectFields)
	{
		...
		if (!empty($arGroupBy) && is_array($arGroupBy))
		{
			...
			foreach ($arGroupBy as $key => $val)
			{
				$val = strtoupper($val);
				$key = strtoupper($key);
				if (isset($arFields[$val]) && !in_array($key, $arGroupByFunct))
				{
		...
		else
		{
				...
				|| in_array("*", $arSelectFields))
			{
				for ($i = 0, $intCount = count($arFieldsKeys); $i < $intCount; $i++)
				{
					...
					{
						if (($strDBType == "ORACLE" || $strDBType == "MSSQL") && isset($arOrder[$arFieldsKeys[$i]]))
				...
				foreach ($arSelectFields as $key => $val)
				{
					...
					if (isset($arFields[$val]))
					{
						...
						else
						{
							...
							{
								if (($strDBType == "ORACLE" || $strDBType == "MSSQL") && isset($arOrder[$val]))
...
*/
	...
	public function OnBeforeCatalogDelete($ID)
	{
		...
		if (CCatalogSKU::TYPE_CATALOG != $arCatalog['CATALOG_TYPE'])
		{
			...
			{
				$arMsg[] = array('id' => 'IBLOCK_ID', 'text' => GetMessage('BT_MOD_CATALOG_ERR_CANNOT_DELETE_SKU_IBLOCK'));
				$obError = new CAdminException($arMsg);
				$APPLICATION->ThrowException($obError);
				return false;
			}
	...
	public static function OnIBlockModuleUnInstall()
	{
		...
		$APPLICATION->ThrowException(GetMessage('BT_MOD_CATALOG_ERR_IBLOCK_REQUIRED'));
		return false;
	}
