...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["BARCODE_AJAX"] === 'Y' && check_bitrix_sessid())
{
		...
		else
		{
			...
			if($addDefault == "Y" || ($bCanBuy && $addDefault == "N"))
			{
				$arParams = "{'id' : '".$arItems["ID"]."',
					...
					'type' : '".CUtil::JSEscape($arCatalogProduct["TYPE"])."',
...
function SelEl(arParams, el)
{
	...
	var allButtons = BX(formId).querySelectorAll('.adm-list-table-cell-last input');
	[].forEach.call(allButtons, function disabler(item) {
		item.disabled = true;
	});
	setTimeout(function(){[].forEach.call(allButtons, function disabler(item) {
		item.disabled = false;
	});}, 2000);
}
...
function checkParameters(f)
{
	...
	[].forEach.call(deleteEmptyInputs, function padParent(item) {
		if(item.value == '')
...
<tr>
	...
	<td valign="top" align="left" style="border-left: 1px solid rgb(164, 185, 204);padding-left:15px;">
		<form name="find_form" method="GET" onsubmit="checkParameters(this); return false;" action="<?echo $APPLICATION->GetCurPage()?>?">
