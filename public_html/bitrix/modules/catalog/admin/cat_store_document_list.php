...
if($arSelectFieldsMap['CREATED_BY'] || $arSelectFieldsMap['MODIFIED_BY'])
{
	...
	foreach($arRows as &$row)
	{
		...
		{
			$strCreatedBy = '';
			if($row->arRes['CREATED_BY'] > 0 && array_key_exists($row->arRes['CREATED_BY'], $arUserList))
			{
		...
		{
			$strModifiedBy = '';
			if($row->arRes['MODIFIED_BY'] > 0 && array_key_exists($row->arRes['MODIFIED_BY'], $arUserList))
			{
