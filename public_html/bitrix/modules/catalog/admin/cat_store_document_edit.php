...
$arCSSInclude = array('css' => '/bitrix/panel/catalog/catalog_store_docs.css');

CJSCore::RegisterExt('catalogDocs', $arCSSInclude);
CUtil::InitJSCore(array("catalogDocs"));
...
if(is_array($arResult["ELEMENT"]))
{
	foreach($arResult["ELEMENT"] as $code => $value)
	{
		...
		{
			$barcodeCount = (isset($value['BARCODE'][0])) ? count(explode(',', $value['BARCODE'][0])) : 0;
		}
		elseif(!$isMultiply)
		{
...
function enterBarcodes(id)
{
	...
	if(BX('CAT_DOC_AMOUNT_HIDDEN_'+id))
		amount = parseInt(BX('CAT_DOC_AMOUNT_HIDDEN_'+id).value);
	else
		amount = 0;
	if(isNaN(amount))
		amount = 0;
