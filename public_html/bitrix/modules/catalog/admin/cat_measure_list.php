...
if(($arID = $lAdmin->GroupAction()) && !$bReadOnly)
{
	...
	{
		$arID = Array();
		$dbResultList = CCatalogMeasure::getList(array($_REQUEST["by"] => $_REQUEST["order"]));
...
while($arRes = $dbResultList->Fetch())
{
	...
	{
		$arRes['USER_ID'] = intval($arRes['USER_ID']);
		if(0 < $arRes['USER_ID'])
...
if($arSelectFieldsMap['USER_ID'] || $arSelectFieldsMap['MODIFIED_BY'])
{
	if(!empty($arUserID))
	{
		$by = 'ID';
		$order = 'ASC';
		$rsUsers = CUser::GetList($by, $order, array('ID' => implode(' || ', array_keys($arUserID))), array('FIELDS' => array('ID', 'LOGIN', 'NAME', 'LAST_NAME')));
