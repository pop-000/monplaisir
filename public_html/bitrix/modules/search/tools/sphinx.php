...
class CSearchSphinx extends CSearchFullText
{
	...
	{
		$result = mysql_query("truncate rtindex ".$this->indexName);
		$this->connect($this->connectionIndex, $this->indexName);
	}

	public function deleteById($ID)
	{
		$result = mysql_query("delete from ".$this->indexName." where id = ".intval($ID));
	}
		...
		if ($strQuery || $this->tags || $bTagsCloud)
		{
			...
			if ($bTagsCloud)
			{
				$sql = "
					...
					,if(date_to, date_to, ".time().") date_to_nvl
					,if(date_from, date_from, ".time().") date_from_nvl
	...
	function searchTitle($phrase = "", $arPhrase = array(), $nTopCount = 5, $arParams = array(), $bNotFilter = false, $order = "")
	{
		...
		if ($checkDates)
		{
			$arWhere[] = "date_from_nvl <= ".(time()-CTimeZone::GetOffset());
			$arWhere[] = "date_to_nvl >= ".(time()-CTimeZone::GetOffset());
		}
		...
		foreach($arFilter as $field=>$val)
		{
			...
			case "CHECK_DATES":
				if($val == "Y")
				{
					if ($inSelect)
					{
						$arWhere[] = "if(date_from, date_from, ".time().") <= ".(time()-CTimeZone::GetOffset());
						$arWhere[] = "if(date_to, date_to, ".time().") >= ".(time()-CTimeZone::GetOffset());
					}
					else
					{
						$arWhere[] = "date_from_nvl <= ".(time()-CTimeZone::GetOffset());
