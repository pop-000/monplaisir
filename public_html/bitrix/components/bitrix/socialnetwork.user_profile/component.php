...
else
{
	...
	else
	{
		...
		$arResult["Urls"]["ExternalMail"] .= ((StrPos($arResult["Urls"]["ExternalMail"], "?") !== false) ? "&" : "?")."STEP=setup";
		...
		if (CModule::IncludeModule('intranet'))
		{
			...
			if($arParams["CACHE_TIME"] == 0 || $obCache->StartDataCache($arParams["CACHE_TIME"], $arResult["User"]["ID"], $path))
			{
				if($arParams["CACHE_TIME"] > 0 && defined("BX_COMP_MANAGED_CACHE"))
				{
					...
					$GLOBALS["CACHE_MANAGER"]->RegisterTag("USER_CARD_".intval($arResult["User"]["ID"] / TAGGED_user_card_size));
				}
