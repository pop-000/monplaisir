...
window.BXfpdPostSelectCallback = function(item, type, search)
{
	if(!BX.findChild(BX('feed-add-post-destination-item-post'), { attr : { 'data-id' : item.id }}, false, false))
	{
		...
		BX('feed-add-post-destination-item-post').appendChild(BX.create("span", { attrs : { 'data-id' : item.id }, props : { className : "feed-add-post-destination feed-add-post-destination-"+type1 }, children: [
				...
				BX.create("span", { props : { 'className' : "feed-add-post-del-but"}, events : {'click' : function(e){BX.SocNetLogDestination.deleteItem(item.id, type, BXSocNetLogDestinationFormNamePost);BX.PreventDefault(e)}, 'mouseover' : function(){BX.addClass(this.parentNode, 'feed-add-post-destination-hover')}, 'mouseout' : function(){BX.removeClass(this.parentNode, 'feed-add-post-destination-hover')}}})
			]}));
	}
...
window.showSharing = function(postId, userId)
{
	...
	if(window["postDest"+postId])
	{
		...
		BX('blg-post-img-'+postId).insertBefore(destForm, BX('blg-post-inform-'+postId));
	...
	if (elements != null)
	{
		...
		for (var j = 0; j < elements.length; j++)
		{
			if(!BX.hasClass(elements[j], 'feed-add-post-destination-undelete'))
			{
				...
				if(type.length > 0)
				{
					window["postDest"+postId].push({id: id, name: name, type: type});
					var destText = BX.create("span", { children: [
								BX.create("span", {html : ', '}),
								BX.create("a", { props : { 'className' : "feed-add-post-destination-new" }, href: '', html : name})
								]});
					if(hiddenDest)
					{
