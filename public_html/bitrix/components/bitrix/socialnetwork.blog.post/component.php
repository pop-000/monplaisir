...
)
{
	if(!empty($arPost))
	{
		...
		if($arResult["PostPerm"] > BLOG_PERMS_DENY)
		{
				...
				if(!empty($arNewRights))
				{
					if(CBlogPost::Update($arParams["ID"], array("SOCNET_RIGHTS" => $perms2update, "HAS_SOCNET_ALL" => "N")))
					{
						...
						if($comId = CBlogComment::Add($arComFields))
						{
							...
							{
								CPullWatch::AddToStack('UNICOMMENTSBLOG_'.$arPost["ID"],
										array(
											...
											'params' => Array(
												...
												"ACTION" => "REPLY", 
												"URL" => array(
													"LINK" => CComponentEngine::MakePathFromTemplate(htmlspecialcharsBack($arParams["PATH_TO_POST"]), array("post_id" => $arParams["ID"], "user_id" => $arPost["AUTHOR_ID"]))."?commentId=".$comId,
													),
			...
			$cache_id = "blog_socnet_post_".md5(serialize($arCacheID))."_".LANGUAGE_ID."_".FORMAT_DATETIME;
			if(($tzOffset = CTimeZone::GetOffset()) <> 0)
				$cache_id .= "_".$tzOffset;
			...
			if (is_array($arResult["POST_PROPERTIES"]["DATA"]["UF_BLOG_POST_IMPRTNT"]) &&
