...
<!-- Description + files -->
<div id="editor-outer-cont-<?=$id?>">
	<div class="feed-event-form feed-event-text" id="editor-cont-<?=$id?>">
...
<div>
	...
	<?
	if (!function_exists("__main_post_form_get_cid_webdav1"))
	{
		function __main_post_form_get_cid_webdav1($arResult = false, $arParams = false)
		{
			static $CID = false;
	...
	<input type="hidden" name="calendar_upload_cid" id="calendar_upload_cid<?=$id?>" value="" />
</div>
...
<div class="feed-event-destination-block">
	...
	<span class="feed-add-destination-input-box" id="feed-event-dest-input-box">
		...
		<script type="text/javascript">
			destinationFormName = 'cal<?=randString(6)?>';
...
window.oEditEventManager = new window.EditEventManager(<?=CUtil::PhpToJSObject(
	array(
		"id" => $id,
		"editorContId" => "editor-cont-".$id,
		"LHEJsObjName" => $arParams["JS_OBJECT_NAME"],
		"LHEId" => $arParams["LHE"]["id"],
		"WDControllerCID" => $arParams["UPLOAD_WEBDAV_ELEMENT_CID"],
		'showTopPanel' => false,
		'parsers' => array("postdocument", "postimage", "postfile"),
		'arFiles' => array_keys($arParams["FILES"]["VALUE_JS"]),
