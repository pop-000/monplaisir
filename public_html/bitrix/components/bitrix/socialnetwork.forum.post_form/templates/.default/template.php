		...
		<div class="forum-reply-field forum-reply-field-text">
			...
			$APPLICATION->IncludeComponent(
				...
				Array(
					...
					"UPLOAD_FILE" => (($arParams["FORUM"]["ALLOW_UPLOAD"] == "N" || array_key_exists("UF_FORUM_MESSAGE_DOC", $arResult["DATA"]["PROPS"])) ? false : array(
						"INPUT_NAME" => 'FILES',
						...
						"MULTIPLE" => "Y",
						"MODULE_ID" => "forum",
						"ALLOW_UPLOAD" => ($arResult["FORUM"]["ALLOW_UPLOAD"] == "Y" ? "I" : $arResult["FORUM"]["ALLOW_UPLOAD"]),
						"ALLOW_UPLOAD_EXT" => $arResult["FORUM"]["ALLOW_UPLOAD_EXT"]
					)),
					"UPLOAD_WEBDAV_ELEMENT" => $arResult["DATA"]["PROPS"]["UF_FORUM_MESSAGE_DOC"],
...
<?
/* ATTACH FILES */
if ($arResult["SHOW_PANEL"]["ATTACH"] == "Y" && empty($arResult["DATA"]["FILES"]) && !array_key_exists("UF_FORUM_MESSAGE_DOC", $arResult["DATA"]["PROPS"])){
?><div class="forum-reply-field forum-reply-field-upload" onclick="this.className+=' forum-reply-field-upload-hover';">
	<a href="javascript:void(0);" id="bx-b-uploadfile-<?=$arParams["FORM_ID"]?>"><?=($arResult["FORUM"]["ALLOW_UPLOAD"]=="Y") ? GetMessage("F_LOAD_IMAGE") : GetMessage("F_LOAD_FILE") ?></a>
</div>
<? };
$res = array_diff_key($arResult["DATA"]["PROPS"], array("UF_FORUM_MESSAGE_DOC" => ""));
if (!!$arResult["DATA"]["PROPS"] && !!$res)
{
	foreach ($res as $FIELD_NAME => $arPostField) { ?>
