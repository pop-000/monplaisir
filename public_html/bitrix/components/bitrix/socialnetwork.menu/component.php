...
if (
	array_key_exists("Urls", $arParams["arResult"]) 
	&& is_array($arParams["arResult"]["Urls"])
	&& array_key_exists("content_search", $arParams["arResult"]["Urls"])
)
	$arParams["arResult"]["Urls"]["search"] = $arParams["arResult"]["Urls"]["content_search"];

if (
	array_key_exists("CanView", $arParams["arResult"]) 
	&& is_array($arParams["arResult"]["CanView"])
	&& array_key_exists("content_search", $arParams["arResult"]["CanView"])
)
	$arParams["arResult"]["CanView"]["search"] = $arParams["arResult"]["CanView"]["content_search"];

if (
	array_key_exists("Title", $arParams["arResult"]) 
	&& is_array($arParams["arResult"]["Title"])
	&& array_key_exists("content_search", $arParams["arResult"]["Title"])
)
...
if ($arParams["ENTITY_TYPE"] == "M")
{
	...
	if (
		array_key_exists("arResult", $arParams)
		&& is_array($arParams["arResult"])
		&& array_key_exists("Urls", $arParams["arResult"])
		&& is_array($arParams["arResult"]["Urls"])
		&& array_key_exists("BizProc", $arParams["arResult"]["Urls"])
	)
