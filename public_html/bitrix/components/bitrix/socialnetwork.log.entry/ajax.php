...
if(CModule::IncludeModule("socialnetwork"))
{
	...
	if (in_array($action, array("add_comment", "get_comment", "get_comments")))
	{
	...
	elseif ($action == "add_comment")
	{
		...
		if ($arLog = CSocNetLog::GetByID($log_id))
		{
			if (
				...
				&& 
				(
					!in_array($arLog["EVENT_ID"], array("crm_lead_message", "crm_deal_message", "crm_company_message", "crm_contact_message"))
			...
			if ($arCommentEvent)
			{
				...
				if ($bCanAddComments)
				{
					...
					//					"blog_post" => array("BLOG", "BG"),
									...
									if (array_key_exists($arLog["EVENT_ID"], $entities_xml_id) && $arLog["SOURCE_ID"] > 0)
	...
	elseif ($action == "get_comments")
	{
		...
		if (strpos($log_entity_type, "CRM") === 0)
		{
			$arListParams = array("IS_CRM" => "Y", "CHECK_CRM_RIGHTS" => "Y");
		}
		else
		...
		if (
			intval($log_tmp_id) > 0
			&& ($rsLog = CSocNetLog::GetList(array(), array("ID" => $log_tmp_id), false, false, array("ID"), $arListParams))
			&& ($arLog = $rsLog->Fetch())
		)
		{
			$arParams = array(
