...
BX.IM = function(domNode, params)
{
	...
	this.revision = 29; // api revieion - check include.php
	this.errorMessage = '';
	...
	this.webrtc = new BX.IM.WebRTC(this, {
		...
		'phoneAvailable': params.webrtc && params.webrtc.phoneAvailable || false,
...
BX.IM.prototype.callTo = function(userId, video)
{
	...
	{
		location.href = "bx://call/"+(video? 'video': 'audio')+"/"+userId;
	}
	else
...
BX.IM.prototype.phoneTo = function(number, params)
{
	...
	if (!this.desktop.ready() && this.desktopStatus && this.desktopVersion >= 18)
	{
		...
		location.href = "bx://call/phone/"+escape(number)+stringParams;
	}
	else
	{
		if (typeof(params) != 'object')
...
BX.IM.prototype.openSettings = function(params)
{
	...
	if (this.messenger.popupMessenger != null && !this.desktop.ready())
	...
	if (!this.desktop.ready() && this.messenger.popupMesseger == null)
		BX.bind(document, "click", BX.proxy(this.autoHide, this));
};
...
BX.IM.prepareText = function(text, prepare, quote, image, highlightText)
{
	...
	if (image)
	{
		textElement = textElement.replace(/<a(.*?)>(.*?)\.(jpg|jpeg|png|gif)<\/a>/ig, function(whole, aInner, text, ext)
		{
			if (text.indexOf("/docs/pub/") > 0)
				return whole;
			else
				return '<span class="bx-messenger-image-box"><a' +aInner+ '><img src="' +text+ '.' +ext+ '" class="bx-messenger-image-text"></a></span>';
		});
	}
	if (highlightText)
	{
...
BX.IM.prototype.storageSet = function(params)
{
	...
	{
		this.stopRepeatSound(params.value.sound);
	}
};
})();
...
BX.Messenger = function(BXIM, params)
{
	...
	this.popupContactListElementsSize = this.desktop.ready()? 319: 279;
...
BX.Messenger.prototype.openMessenger = function(userId)
{
	...
	if (this.desktop.ready() && BX.desktop.currentTab != 'im')
	{
	...
	this.popupMessengerContent = BX.create("div", { props : { className : "bx-messenger-box"+(this.webrtc.callInit? ' bx-messenger-call': '') }, style: styleOfContent, children : [
		...
		this.popupContactListWrap = BX.create("div", { props : { className : "bx-messenger-box-contact" }, style : {width: this.popupContactListSize+'px'},  children : [
			...
			BX.create("div", { props : { className : "bx-messenger-input-search"+(this.webrtc.phoneEnabled && !this.desktop.ready()? ' bx-messenger-input-search-phone': '') }, children : [
			...
			this.desktop.ready()? null: BX.create('div', {props : { className : "bx-messenger-cl-notify-wrap" }, children : [
			...
			BX.create('div', {props : { className : "bx-messenger-cl-panel" }, children : [ BX.create('div', {props : { className : "bx-messenger-cl-panel-wrap" }, children : [
				...
				BX.create('span', {props : { className : "bx-messenger-cl-panel-right-wrap" }, children : [
					this.contactListPanelSettings = this.desktop.ready()? null: BX.create("span", { props : { title : BX.message("IM_SETTINGS"), className : "bx-messenger-cl-panel-settings-wrap"}})
				]})
			]}) ]})
		]}),
		/* DIALOG */
	...
	if (this.contactListPanelSettings)
		BX.bind(this.contactListPanelSettings, "click", BX.delegate(function(){this.openSettings()}, this.BXIM));
			...
			BX.create("a", {
				...
				events : {
					click: BX.delegate(function(e){
						...
						if (this.webrtc.callInit || !currentChat.call_entity_type)
							return false;

						if (!currentChat.call_entity_type || currentChat.call_entity_type == 'CALL')
						{
							this.BXIM.phoneTo('+'+currentChat.call_number, {"ENTITY_TYPE": currentChat.call_entity_type, "ENTITY_ID": currentChat.call_entity_id});
						}
						else
						{
							this.openPopupMenu(BX.proxy_context, 'phoneMenu', true, currentChat);
						}
						BX.PreventDefault(e);
					}, this)
				}
...
BX.Messenger.prototype.openPopupMenu = function(bind, type, setAngle, params)
{
	...
	else if (type == 'phoneMenu')
	{
		offsetTop = 2;
		offsetLeft = 12;

		menuItems = [
			{text: BX.message('IM_PHONE_CALL_RECORD'), onclick: BX.delegate(function(){ this.BXIM.phoneTo('+'+params.call_number, {"ENTITY_TYPE": params.call_entity_type, "ENTITY_ID": params.call_entity_id, "RECORDING": "Y"}); }, this)},
			{text: BX.message('IM_PHONE_CALL_NO_RECORD'), onclick: BX.delegate(function(){ this.BXIM.phoneTo('+'+params.call_number, {"ENTITY_TYPE": params.call_entity_type, "ENTITY_ID": params.call_entity_id, "RECORDING": "N"}); }, this)}
		];
	}
	...
	else if (type == 'dialog')
	{
		...
		menuItems = [
			...
			selectedText.length <= 0? null: {text: BX.message("IM_MENU_QUOTE"), onclick: BX.delegate(function(){ this.insertQuoteText(messageName, messageDate, selectedText); this.closeMenuPopup(); }, this)},
...
BX.IM.Desktop = function(BXIM, params)
{
	...
	else if (this.run() && this.BXIM.init)
	{
		...
		BX.desktop.addCustomEvent("bxCallDecline", BX.delegate(function() {
			...
			this.webrtc.callDeclineSelfDisabled = true;
...
{
	return typeof(BX.desktop) != 'undefined' && this.BXIM.desktopStatus;
};
...
BX.IM.Desktop.prototype.openCallFloatDialog = function()
{
	...
	this.openTopmostWindow("", 'BXIM.webrtc.callFloatDialog("'+BX.util.htmlspecialchars(this.webrtc.callOverlayTitleBlock.innerHTML)+'", "'+(this.webrtc.callVideo? this.webrtc.callOverlayVideoMain.src: '')+'", '+(this.webrtc.audioMuted?1:0)+')', {}, 'im-desktop-call');
};
...
BX.IM.Desktop.prototype.openTopmostWindow = function(html, js, initJs, bodyClass)
{
	if (!this.ready())
		return false;

	this.closeTopmostWindow();
	this.topmostWindow = BXDesktopSystem.ExecuteCommand('topmost.show.html', this.getHtmlPage(html, js, initJs, bodyClass));
};
				...
				if (params.command == 'join')
				{
					...
					else
					{
						...
						if (this.desktop.ready() && !this.BXIM.windowFocus)
						{
							...
							this.desktop.openTopmostWindow("", "BXIM.webrtc.callNotifyWaitDesktop("+params.chatId+","+params.senderId+", "+(params.video?1:0)+", "+(params.callToGroup?1:0)+", true);", data, 'im-desktop-call');
						}
					}
				}
				else if (params.command == 'invite' || params.command == 'invite_join')
				{
					...
					else
					{
						...
						if (this.desktop.ready() && !this.BXIM.isFocus('all'))
						{
							...
							this.desktop.openTopmostWindow("", "BXIM.webrtc.callNotifyWaitDesktop("+params.chatId+","+params.senderId+", "+(params.video?1:0)+", "+(params.callToGroup?1:0)+");", data, 'im-desktop-call');
						}
					}
				}
...
{
	if (!params || !(params.toUserId || params.toPhoneNumber) || !params.fromUserId || !params.buttons)
		return false;
...
BX.IM.WebRTC.prototype.callUserOverlayShow = function(params)
{
	...
	if (params.toPhoneNumber || params.callTitle)
	{
		this.callToPhone = true;
		var callTitle = BX.message('IM_PHONE_CALL_VOICE_TO').replace('#PHONE#', (params.callTitle? params.callTitle: '+'+params.toPhoneNumber));
	}
	else
	{
		var callTitle = this.callOverlayTitle();
	...
	return [
		...
		BX.create("div", { props : { className : 'bx-messenger-call-overlay-photo'}, children: [
			BX.create("div", { props : { className : 'bx-messenger-call-overlay-photo-left'}, children: [
				BX.create("div", { props : { className : 'bx-messenger-call-overlay-photo-block'}, children: [
					this.callOverlayPhotoCompanion = BX.create("img", { props : { className : 'bx-messenger-call-overlay-photo-img'}, attrs : { 'data-userId': params.toPhoneNumber? 'phone': callUserId, src : this.getHrPhoto(params.toPhoneNumber? 'phone': callUserId)}})
				]})
			]}),
...
BX.IM.WebRTC.prototype.callOverlayToggleSize = function(minimize)
{
	...
	this.desktop.openCallFloatDialog();
	this.desktop.closeCallFloatDialog();
...
BX.IM.WebRTC.prototype.callOverlayUpdatePhoto = function()
{
	...
	if (this.callOverlayPhotoCompanion)
	{
		...
		else if (this.messenger.users[companionUserId].avatar == '/bitrix/js/im/images/blank.gif')
...
BX.IM.WebRTC.prototype.openKeyPad = function(e)
{
	...
	if (this.callActive && this.desktop.ready() && BX.hasClass(this.callOverlay, 'bx-messenger-call-overlay-line'))
	{
		BX.desktop.closeTab('im-phone');
		return false;
	}
	...
	this.popupKeyPad = new BX.PopupWindow('bx-messenger-popup-keypad', bindElement, {
		...
		offsetLeft: offsetLeft,
		closeByEsc: true,
		angle : { position : this.desktop.ready() && !this.callActive? "left": "top", offset: this.desktop.ready()? (this.callActive? 120: 76): 92 },
		autoHide: true,
		zIndex: 200,
		events : {
			onPopupClose : function() { this.destroy() },
			...
			buttons : [
				{
					...
					events: {
						click : BX.delegate(function() {
							if (this.callOverlayUserId || this.callOverlayChatId)
								this.phoneCommand('message', {'COMMAND': 'decline_self', 'DIALOG_ID': this.callOverlayUserId? this.callOverlayUserId: 'chat'+this.callOverlayChatId, 'NUMBER_USER': BX.util.htmlspecialcharsback(this.phoneNumberUser)});
							this.phoneCallFinish();
							this.callAbort();
							this.callOverlayClose();
						}, this)
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(data)
				{
					...
					if (data.ERROR == '')
					{
						...
						this.phoneAvailable = data.ENABLE? true: false;
...
BX.IM.WebRTC.prototype.phoneStartCall = function()
{
	if (!this.phoneSupport())
		return false;
...
{
	var result = this.phoneAPI.connected();
	this.phoneLog('Connection established', result);
...
{
	this.phoneParams['CALLER_ID'] = this.phoneCallerID;
	...
	this.phoneLog('Create Call', this.phoneAPI.calls.length);

	var entityType = this.phoneParams['ENTITY_TYPE']? this.phoneParams['ENTITY_TYPE']: 'CALL';
	var entityID = this.phoneParams['ENTITY_ID']? this.phoneParams['ENTITY_ID']: this.phoneNumber;
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_PHONE' : 'Y', 'COMMAND': 'init', 'NUMBER' : this.phoneNumber, 'NUMBER_USER' : BX.util.htmlspecialcharsback(this.phoneNumberUser), 'ENTITY_TYPE':entityType, 'ENTITY_ID':entityID, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.bitrix_sessid()},
		onsuccess: BX.delegate(function(data){
			if (data.ERROR == '')
			{
				if (data.HR_PHOTO)
...
BX.IM.WebRTC.prototype.phoneOnCallConnected = function(e)
{
	...
	this.phoneCommand('message', {'COMMAND': 'start', 'DIALOG_ID': this.callOverlayUserId? this.callOverlayUserId: 'chat'+this.callOverlayChatId, 'NUMBER_USER': BX.util.htmlspecialcharsback(this.phoneNumberUser)});
}
...
BX.IM.WebRTC.prototype.phoneOnCallFailed = function(e)
{
	...
	if (e.code == 603 && this.phoneRinging == 0)
	{
		...
		this.phoneCommand('message', {'COMMAND': 'busy', 'DIALOG_ID': this.callOverlayUserId? this.callOverlayUserId: 'chat'+this.callOverlayChatId, 'NUMBER_USER': BX.util.htmlspecialcharsback(this.phoneNumberUser)});
	}
	else if (e.code == 603 && this.phoneRinging > 0)
	{
		reason = BX.message('IM_M_CALL_ST_DECLINE');
		this.phoneCommand('message', {'COMMAND': 'decline', 'DIALOG_ID': this.callOverlayUserId? this.callOverlayUserId: 'chat'+this.callOverlayChatId, 'NUMBER_USER': BX.util.htmlspecialcharsback(this.phoneNumberUser)});
	}
	else if (e.code == 408)
	{
		reason = BX.message('IM_PHONE_NO_ANSWER');
		this.phoneCommand('message', {'COMMAND': 'no_answer', 'DIALOG_ID': this.callOverlayUserId? this.callOverlayUserId: 'chat'+this.callOverlayChatId, 'NUMBER_USER': BX.util.htmlspecialcharsback(this.phoneNumberUser)});
	}
	else if (e.code == 403)
	{
		...
		try { this.phoneAPI.disconnect(); } catch (e) {}
	}
...
{
	if (this.callActive)
		this.phoneCommand('message', {'COMMAND': 'end', 'DIALOG_ID': this.callOverlayUserId? this.callOverlayUserId: 'chat'+this.callOverlayChatId, 'NUMBER_USER': this.phoneNumberUser});

	if (this.phoneCurrentCall)
	{
		try { this.phoneCurrentCall.hangup(); } catch (e) {}
		this.phoneCurrentCall = null;
		this.phoneLog('Call disconnect');
	}

	if (false && this.phoneAPI) // TODO REMOVE false !
	{
		try { this.phoneAPI.disconnect(); } catch (e) {}
		this.phoneLog('Connection close');
	}
	if (this.popupKeyPad)
		this.popupKeyPad.close();
