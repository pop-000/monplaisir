...
if (CModule::IncludeModule("iblock"))
{
	...
	{
		$dbSections = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arIBlock["ID"]), true, array("ID", "SECTION_PAGE_URL", "PICTURE", "DESCRIPTION", "UF_ICON"));
		while($arSections = $dbSections->GetNext())
		{
			$pictureSrc = CFile::GetFileArray($arSections["PICTURE"]);
			$iconSrc = CFile::GetFileArray($arSections["UF_ICON"]);
			...
			$arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["ICON"] = $iconSrc["SRC"];
			$arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["DESCRIPTION"] = $arSections["DESCRIPTION"];
			$arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["ELEMENT_CNT"] = $arSections["ELEMENT_CNT"];
		}
	}
}
...
foreach($arResult as $arItem)
{
	...
	$arItem["PARAMS"]["icon_src"] = $arSectionsInfo[$arItem["PARAMS"]["item_id"]]["ICON"];
	$arItem["PARAMS"]["description"] = $arSectionsInfo[$arItem["PARAMS"]["item_id"]]["DESCRIPTION"];
// фильтр: только непустые категории ( "ELEMENT_CNT" > 0 )
	if( $arSectionsInfo[$arItem["PARAMS"]["item_id"]]["ELEMENT_CNT"] > 0 || $arItem["IS_PARENT"]){
		if ($arItem["DEPTH_LEVEL"] == "1")
		{
